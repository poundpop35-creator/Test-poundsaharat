...
(() => {
  ...
  function renderQuestion() {
    ...
    $('qCard').innerHTML = `<p class="small">${esc(item.topicTitle)}</p><h3 id="questionTitle" tabindex="-1">${item.q}</h3><div id="options">${item.options.map((o, i) => `<button class="option" type="button" data-action="pick" data-id="${i}"><span>${['ก','ข','ค','ง'][i]}.</span>${o.text}</button>`).join('')}</div><div id="explanation" aria-live="polite"></div>`;
  ...
  function pick(index) {
    ...
    $('explanation').innerHTML = session.mode === 'mock' ? '<p>บันทึกคำตอบแล้ว ดูเฉลยพร้อมกันเมื่อจบชุด</p>' : `<h4>${correct ? '✓ ถูกต้อง' : 'ยังไม่ถูก • คำตอบที่ถูก: ' + item.options[item.answer].text}</h4><p>${item.e}</p><p class="small">${esc(item.reference || '')} ${links(item.sources)}</p>`;
  ...
  function result() {
    ...
    $('result').innerHTML = `<div class="panel"><h2>ผลการฝึก ${session.score}/${total} ข้อ (${Math.round(session.score / total * 100)}%)</h2><p>ทบทวนเหตุผลของข้อที่พลาด แล้วฝึกซ้ำเพื่อเช็กความเข้าใจ</p><p class="small">คะแนนจากชุดฝึกนี้ไม่ใช่เกณฑ์ผ่านหรือการรับรองความพร้อมสอบจริง</p><div class="actions">${button('retry', 'ทำชุดเดิมอีกครั้ง', '', true)}${button('wrong', 'ทวนข้อที่ยังผิด')}${button('home', 'กลับบทเรียน')}</div></div>
      ...
      <h3>เฉลยทุกข้อและเหตุผล</h3>${session.responses.map((r, i) => `<details class="review" ${r.correct ? '' : 'open'}><summary>${r.correct ? '✓' : '✗'} ข้อ ${i + 1}: ${r.item.q}</summary><p>คำตอบของฉัน: ${r.item.options[r.index].text}</p><p><b>คำตอบที่ถูก: ${r.item.options[r.item.answer].text}</b></p><p>${r.item.e}</p><p class="small">${esc(r.item.reference || '')} ${links(r.item.sources)}</p>${button('read', 'กลับไปอ่านหัวข้อนี้', r.item.topicId)}</details>`).join('')}`;
    show('result');
  }
  function sourcePage() {
    session = null;
    const ids = [...new Set(topics.flatMap(t => t.sources))];
    $('sourcesBody').innerHTML = `<h2>ขอบเขตสอบและแหล่งอ่าน</h2><p>${esc(config.scopeNote)}</p><p>ภาพระบุคะแนนเต็ม 200 คะแนน ไม่ได้ระบุจำนวนข้อ สัดส่วนคะแนนรายหัวข้อ หรือยืนยันว่าทุกข้อเป็นปรนัย ข้อฝึกในแอปแต่งขึ้นเพื่อเรียนรู้ ไม่ใช่ข้อสอบจริงของหน่วยงาน</p><ol>${official.map(t => `<li>${esc(t.title)}</li>`).join('')}</ol><h3>อ่านกฎหมายและเอกสารฉบับเต็ม</h3>${fullLinks(ids)}<h3>แหล่งเผยแพร่และคำอธิบาย • ตรวจแหล่ง 9 ก.ย. 2569</h3>${ids.map(id => `<section><h4>${links([id])}</h4><p>${esc(sources[id].note || '')}</p></section>`).join('')}<p class="small">ข่าวระบุวันที่ของเหตุการณ์และมีลิงก์ติดตามฉบับใหม่ เนื้อหาไม่ได้อัปเดตอัตโนมัติ</p>`; show('sources');
  }
